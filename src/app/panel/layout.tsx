...
const navLinks = [
  { name: "Explore", icon: Compass, href: "/panel" },
  { name: "Surveys", icon: ClipboardList, href: "/panel/surveys" },
  { name: "Wallet", icon: Wallet, href: "/panel/wallet" },
  { name: "Help Center", icon: HelpCircle, href: "/panel/help" },
];
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      >
        ...
        <div className="px-5 mb-10 overflow-hidden">
           <div className={`p-4 rounded-3xl bg-gray-50 border border-gray-100 transition-all ${isSidebarOpen ? "opacity-100" : "opacity-0 invisible"}`}>
              <div className="flex items-center gap-3 mb-3">
                 <div className="w-8 h-8 rounded-full bg-brand-primary/10 flex items-center justify-center text-brand-primary">
                    <TrendingUp size={14} />
                 </div>
                 <span className="text-[10px] font-black uppercase tracking-widest text-gray-500">Tier: Platinum</span>
              </div>
            ...
            return (
              ...
              >
                ...
                {isActive && isSidebarOpen && (
                  <motion.div layoutId="activePill" className="absolute right-4 w-1 h-1 bg-white rounded-full" />
                )}
              </Link>
            );
          })}
        ...
        <div className="p-6 mt-auto">
          <div className={`mb-6 p-4 rounded-3xl bg-brand-primary text-white relative overflow-hidden group cursor-pointer ${isSidebarOpen ? "block" : "hidden"}`}>
          ...
          <Link href="/panel/settings" className="flex items-center gap-4 px-5 py-4 w-full text-gray-400 hover:text-gray-900 transition-colors">
            <Settings size={20} />
      ...
      <nav className="lg:hidden fixed bottom-6 left-6 right-6 h-20 bg-white/80 backdrop-blur-2xl border border-white/20 rounded-[2.5rem] shadow-2xl z-[100] px-8 flex items-center justify-between">
         {navLinks.map((link) => {
           ...
           const Icon = link.icon;
           return (
             <Link key={link.name} href={link.href} className={`relative flex flex-col items-center gap-1 ${isActive ? "text-brand-primary" : "text-gray-400"}`}>
                <Icon size={24} />
                {isActive && <motion.div layoutId="mobileActive" className="w-1 h-1 bg-brand-primary rounded-full mt-1" />}
             </Link>
           );
         })}
