...
export function VendorCallbackFields<T extends FieldValues>({
  ...
  return (
    <div className="space-y-6">
      <div className="rounded-xl border border-brand-primary/15 bg-brand-primary/5 p-4 text-sm text-gray-700">
        ...
        {VENDOR_CALLBACK_OUTCOMES.map((outcome) => {
          ...
          return (
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-700 font-bold">{config.label}</FormLabel>
                  <FormControl>
                    <Input
                      type="url"
                      inputMode="url"
